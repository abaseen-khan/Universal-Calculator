...
<script>
    ...
    for (var i = 0; i <= 10000000000; i++){
        var answer = readLine("Would you like to calculate sales tax on a purchase, or the tip for an order? ");
        if (answer.toUpperCase().includes("TIP")){
            calculateTip();
            break;
        }
        else if (answer.toUpperCase().includes("TAX")){
            calculateTax();
            break;
        }
        else {
...
function calculateTip(){
    var subtotal = readFloat("What is the subtotal of the order? ");
    var percent = readFloat("What percentage tip would you like to leave? ");
    var tip = subtotal * (percent / 100);
    var total = subtotal + tip;
    println("Here is your receipt:");
    createReceipt(subtotal, tip, total);
}

function calculateTax(){
    var subtotal = readFloat("What is the subtotal of the purchase? ");
    var tax = subtotal * 0.08875;
    var total = subtotal + tax;
    println("Here is your receipt:");
    createReceipt(subtotal, tax, total);
}

function createReceipt(subtotal, percentage, total){
    println("");
    println("_______________________________");
    println("           RECEIPT             ");
    println("Subtotal:              $" + Math.round(subtotal * 100) / 100);
    println("Additional fees:       $" + Math.round(percentage * 100) / 100);
    println("-------------------------------");
    println("Total:                 $" + Math.round(total * 100) / 100);
    println("-------------------------------");
    var signature = readLine("Sign here:             ");
    println("_______________________________");
